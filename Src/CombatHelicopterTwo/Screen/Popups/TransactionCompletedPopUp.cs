...
// Type: Helicopter.Screen.Popups.TransactionCompletedPopUp
...
{
  internal class TransactionCompletedPopUp : GameScreen
  {
    ...
    public TransactionCompletedPopUp() => this.IsPopup = true;

    public override void LoadContent()
    {
      this._root.Size = new Vector2(800f, 480f);
      TexturedControl child1 = new TexturedControl(ResourcesManager.Instance.GetSprite("PopUpWindow/transactionCompletedPopUp"), Vector2.Zero);
      child1.Position = new Vector2(400f, 240f) - child1.Size / 2f;
      this._root.AddChild((BasicControl) child1);
      this._root.AddChild((BasicControl) new TextControl("Your purchase is done!", ResourcesManager.Instance.GetResource<SpriteFont>("fonts/coalition14"), new Color(254, 224, 23), new Vector2(400f, 260f))
      {
